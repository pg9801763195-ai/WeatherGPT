...
alongside dense sentence embeddings and BM25 lexical retrieval.
"""
import os
import json
import re
...
class WeatherRAGRetriever:
    ...
    def _get_embedding(self, text: str) -> List[float]:
        ...
        # Fast deterministic hash-based embedding fallback
    ...
    def retrieve(self, query: str, top_k: int = 3, category: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        if self.qdrant_client:
            try:
                ...
                # Search across all available Qdrant collections
                collections_to_search = ["sih_climate_archive_kb", "weather_climate_kb"]
                existing_cols = [c.name for c in self.qdrant_client.get_collections().collections]
                            ...
                            if search_res:
                                for hit in search_res:
                                    ...
                                    results.append({
                                        ...
                                        "topic": payload.get("topic", payload.get("city", "Climate Climatology")),
                                        "category": payload.get("category", "General"),
                                        "city": payload.get("city", ""),
                                        "state": payload.get("state", ""),
                                        "temp_avg": payload.get("temp_avg"),
                                        "temp_max": payload.get("temp_max"),
                                        "temp_min": payload.get("temp_min"),
                                        "source_report": payload.get("source", payload.get("source_report", "Qdrant SIH Archive")),
                                        "score": float(hit.score),
                                        "engine": f"Qdrant Vector DB ({col})"
                                    })
                        except Exception:
                            pass
