...
export default function OverviewTab() {
  ...
  const effectiveCondition = currentCity?.condition || 'Clear Sky';
  const activeVisual = resolveWeatherVisual(effectiveCondition);
  
  const displayTempC = currentCity?.tempC ?? 20;
  const displayTempF = currentCity?.tempF ?? 68;
  ...
  return (
    <main className="max-w-7xl mx-auto px-gutter md:px-container-padding-desktop pb-section-gap space-y-8">
      ...
      <section 
        ...
        className="mt-6 md:mt-8 relative rounded-2xl overflow-hidden min-h-[52vh] flex flex-col justify-end p-6 sm:p-8 md:p-12 shadow-md group border border-outline-variant/20 transition-all cursor-default"
      >
        ...
        <div className="relative z-10 text-white space-y-3 pt-8">
          <div className="flex items-center gap-2 text-white/85 text-xs font-medium tracking-wide">
            ...
            <span className="text-sky-300 font-medium">{activeVisual.tagline}</span>
          </div>
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div>
              <h1 className="font-tight font-medium text-headline-lg-mobile md:text-headline-lg text-white tracking-tight drop-shadow-sm">
                Good day, {currentCity?.name || 'Location'}.
              </h1>
              ...
              <div className="flex items-baseline gap-4 mt-2">
                ...
                <div className="space-y-1">
                  ...
                  <span className="text-xs text-white/85 font-normal block drop-shadow-sm">
                    Feels like {formatTemp(currentCity?.feelsLikeC || displayTempC, currentCity?.feelsLikeF || displayTempF)} · UV {currentCity?.uvIndex ?? 5} ({currentCity?.uvLabel || 'Mod'}) · Humidity {currentCity?.humidity ?? 65}% · Wind {currentCity?.windKm ?? 10} km/h
                  </span>
                </div>
              </div>
            </div>
        ...
        >
          <div className="flex items-start gap-4">
            <span className="material-symbols-outlined text-tertiary text-2xl mt-0.5" style={{ fontVariationSettings: "'FILL' 1" }}>
              {currentCity.alert.icon || 'warning'}
            </span>
            <div>
              <div className="flex items-center gap-2 mb-1">
                <span className="px-2 py-0.5 rounded-md bg-tertiary/10 text-tertiary text-[10px] font-bold uppercase tracking-wider font-label-caps">
                  {currentCity.alert.severityLabel || 'Severe Warning'}
                </span>
                <span className="text-xs text-on-tertiary-fixed-variant/75 font-medium">{currentCity.alert.timing}</span>
              </div>
              <h3 className="font-tight text-lg text-on-tertiary-fixed-variant font-semibold group-hover:underline">
                {currentCity.alert.title}
              </h3>
              <p className="font-body-md text-xs text-on-tertiary-fixed-variant/80 mt-1">
                {currentCity.alert.shortDesc}
              </p>
            </div>
          </div>
      ...
      <div className="bg-surface rounded-2xl border border-outline-variant/15 p-7 shadow-sm">
        <div className="flex items-center justify-between mb-3.5">
          <div className="flex items-center gap-2">
            <span className="material-symbols-outlined text-primary text-base">auto_awesome</span>
            <span className="font-label-caps text-xs text-primary uppercase tracking-wider font-bold">Today's Weather Outlook</span>
          </div>
          <button 
        ...
        <section className="space-y-3">
          <div className="flex items-center justify-between">
            <h2 className="font-label-caps text-xs text-on-surface-variant uppercase tracking-wider font-semibold">
              Your Day at a Glance · {currentCity.name}
            </h2>
            <span className="text-[11px] text-on-surface-variant/75 font-medium">Personalized for local activity</span>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {currentCity.dayAtAGlance.map(item => (
              <div 
                key={item.id}
                onClick={() => {
