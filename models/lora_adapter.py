...
if HAS_TORCH:
    class LoRALinear(nn.Module):
        ...
        def __init__(
            ...
            # Frozen Base Pretrained Linear Layer W0
            ...
            # Move base layer to target device/dtype
            ...
            if self.r > 0:
                ...
                # Matrix A: (r, in_features) initialized with Kaiming uniform / Gaussian
                self.lora_A = nn.Parameter(torch.empty((self.r, in_features), dtype=self.target_dtype, device=self.target_device))
                nn.init.kaiming_uniform_(self.lora_A, a=math.sqrt(5))
                
                # Matrix B: (out_features, r) initialized to zeros (so Delta W = 0 at start)
        ...
        def forward(self, x: Any) -> torch.Tensor:
            ...
            # Ensure input is a torch.Tensor on correct device and dtype
            ...
            # 1. Pretrained Base Layer Output
            base_out = self.base_layer(x)

            # 2. LoRA Low-Rank Delta Contribution
            if self.r > 0 and not self.merged and self.lora_A is not None and self.lora_B is not None:
                # x @ lora_A.T -> (..., r)
                # (x @ lora_A.T) @ lora_B.T -> (..., out_features)
    ...
    # NumPy Fallback if PyTorch is not present
    class LoRALinear:
...
def apply_lora_to_model(
    ...
    memory_footprint_mb = (total_trainable_params * 4) / (1024 * 1024)  # FP32 in MB
